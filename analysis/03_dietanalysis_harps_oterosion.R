...
# * otolith erosion ----
erosion <- erosion %>%
  rename(codeotdigstage = Code.OtDigStage,
...
diet <- merge(diet, erosion, all.x = TRUE)
...
ids <- diet %>%
  ...
  filter(digestivetractsection == 'Main Stomach') %>%
  filter(nafo %in% nafodiet) %>%
  filter(year > 1984) %>%
 # filter(subunitarea == 340) %>%
  filter(totalpreyweight > 5000) %>%
  select(idsex)



diet <- diet[which(diet$idsex %in% ids$idsex),]

diet %>%
  filter(preycode == 438) %>%
  group_by(codeotdigstage) %>%
  summarise(number = n())
...
wp <- wp + facet_grid(nafo ~  season + area, drop = TRUE)
...
save_plot("output/harp_diet_morethan5kg.png", wp, base_width = 21, base_height = 10)#, dpi = 900) # make room for figure legend)
