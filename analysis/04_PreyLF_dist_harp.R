...
## read data  ----
load('interimsteps/diet_postanalysis_harp.rdata')
...
preyplotdata <- preyplotdata[which(preyplotdata$mmspcode %in% mammalsp), ]
...
preyplotdata$preyc <- ifelse(preyplotdata$preycat == 'arctic cod', 'A',
                             ifelse(preyplotdata$preycat == 'capelin', 'B',
                                ifelse(preyplotdata$preycat == 'atlantic herring', 'C',
                                       ifelse(preyplotdata$preycat == 'atlantic cod', 'D',
                                              ifelse(preyplotdata$preycat == 'shrimp', 'E', 'F')))))
labels <- c(A = "arctic cod", B = "capelin", C = 'atlantic herring', D = 'atlantic cod', E = 'shrimp')

colores <- c(rgb(0,115/255,76/255),rgb(1,85/255,0))
colores <- c('#00734C', '#FF5500')

preyplot <- ggplot(subset(preyplotdata, preycat != 'shrimp'), aes(predlength, fill = mmsp, colour = mmsp))
...
preyplot <- preyplot + theme(legend.position = "none")
...
preyplot <- preyplot + scale_y_continuous(limits = c(0,0.15), breaks = c(0,0.15), minor_breaks = c(0.1))
...
preyplotnafo <- preyplot + facet_grid(nafo ~ preycat, drop = TRUE)
preyplot <- preyplot + facet_wrap( ~preyc, drop = TRUE,  labeller = labeller(preyc = labels))

preyplot
preyplotnafo

save_plot("output/Prey_LF_harp.png", preyplot, base_width = 10, base_height = 10)#, dpi = 900) # make room for figure legend)
save_plot("output/Prey_LF_nafo_harp.png", preyplotnafo, base_width = 10, base_height = 10)#, dpi = 900) # make room for figure legend)
...
sampsize.nafomonth <- ddply(diet[which(diet$mmspcode %in% mammalsp),],
                            .(nafo,month,mmsp, area, season),
                            summarize,
                            n = length(unique(idsex)))


sampsize.nafomonth <- sampsize.nafomonth[which(sampsize.nafomonth$nafo %in% nafodiet),]
sampsize.nafomonth <- na.omit(subset(sampsize.nafomonth, month < 13))
...
ssp <- ssp + theme_bw()
ssp <- ssp + facet_grid(nafo ~ area + season, drop = TRUE,scales = "free_y")
ssp <- ssp + ggtitle("PG sample size")
ssp <- ssp + theme(plot.title = element_text(size = 15, face = "bold"),
                 strip.text = element_text(size = 12),
                 legend.text = element_text(size = 11),
                 axis.text = element_text(size = 10),
                 axis.title = element_text(size = 13, face = "bold"))
ssp
save_plot("output/harp_samplesizes.png", ssp, base_width = 21, base_height = 10)#, dpi = 900) # make room for figure legend)
